...
`,
  },
  {
    ...
    meta: "components/ui/Stepper.tsx",
    code: `import Stepper from "@/components/custom/Stepper";
import React from "react";
...
const CheckoutStepper: React.FC<CheckoutStepperProps> = ({ stepsData }) => {
  ...
  const ActiveComponent = stepsData[currentStep - 1]
    ...
    <div className="p-5 flex flex-col">
      <div className="flex justify-center items-center relative gap-[20%]">
        {stepsData.map((step, ind) => (
          ...
          >
            <div
              className={\`w-10 h-10 flex justify-center items-center rounded-full border-2 z-50 \${currentStep === ind + 1 ? "bg-black text-white dark:bg-white dark:text-black" : ""} \${currentStep > ind + 1 || isCompleted ? "bg-green-500 text-white" : ""}\`}
            >
