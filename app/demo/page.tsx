...
const SelectableGrid = ({ rows, cols }: { rows: number; cols: number }) => {
  ...
  const handleMouseEnter = (boxNum: number) => {
    if (isMouseDown) {
  ...
  return (
    ...
    >
      {Array.from(Array(rows * cols).keys()).map((_, index) => (
        <div
          key={index}
          className={`flex w-8 h-8 border-2 p-5 border-black justify-center items-center ${
            !selectedGrid.includes(index + 1) ? "bg-green-500" : "flex"
          }`}
